    ...
    const fetchReferrals = async () => {
      ...
      try {
        ...
        let { data } = await API.graphql(graphqlOperation(listReferrals, {
          limit: 900000, 
          filter: { referralPartnerAgentID: { eq: sub}}
        }))
        ...
        if(data.listReferrals.items.length > 0) {
          for(let i = 0; i < data.listReferrals.items.length; i++) {
            ...
            totalIncomeLoop = totalIncomeLoop + data.listReferrals.items[i].referralPartnerAgentPayoutActual === null ? 0 : data.listReferrals.items[i].referralPartnerAgentPayoutActual
            }
          }
