...
function GetPaymentInfo(props) {
  ...
  let stripePromise = loadStripe(process.env.STRIPE_KEY_AZ === undefined ? process.env.REACT_APP_STRIPE_KEY_AZ : process.env.STRIPE_KEY_AZ);
  if(props.globalUser.userRoostedLicenses.items[0].licenseState === 'AZ') {
    stripePromise = loadStripe(process.env.STRIPE_KEY_AZ === undefined ? process.env.REACT_APP_STRIPE_KEY_AZ : process.env.STRIPE_KEY_AZ);
  } else if(props.globalUser.userRoostedLicenses.items[0].licenseState === 'CA') {
    stripePromise = loadStripe(process.env.STRIPE_KEY_CA === undefined ? process.env.REACT_APP_STRIPE_KEY_CA : process.env.STRIPE_KEY_CA);
  }
  ...
  const onSubmit = async (dataCollected) => {
    ...
    try{
      ...
      if(promoCodeValidResponse.promoCodeValid === true) {
        ...
        setPromoCode(promoCodeValidResponse.promoData)
        setOpen(false)
        setLoading(false)
      } else {
        ...
        try {
          ...
          let custom = { 
            ...
            body: {
              amount: 2500,
              currentcy: 'usd',
              stripeState: props.license.licenseState,
