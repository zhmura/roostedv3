...
function SignICA(props) {
  ...
  const [contract, setContract] = useState(<AZIndependentContractorReact history={null} scenario={'setup'} dataCollected={dataCollected} setDataCollected={setDataCollected}/>)
  ...
  useEffect(() => {
    ...
    const selectContractState = () => {
      if(props.globalUser.userRoostedLicenses.items[0].licenseState === 'AZ') {
        setContract(<AZIndependentContractorReact history={null} scenario={'setup'} dataCollected={dataCollected} setDataCollected={setDataCollected}/>)
      }
    }
    selectContractState();
