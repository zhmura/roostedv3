...
function Results({ className, columns, data, history, ...rest }) {
  ...
  const {
    ...
    pageOptions,
    page, //use this in place of rows when using pagination
    state: { 
      pageIndex, 
      //pageSize 
    },
              ...
              {headerGroups.map(headerGroup => (
                <TableRow {...headerGroup.getHeaderGroupProps()}>
                  {headerGroup.headers.map(column => (
                    <TableCell align='center' size='small' {...column.getHeaderProps(column.getSortByToggleProps())}
                    className={
                      column.isSorted
                        ? column.isSortedDesc
                          ? "sort-desc"
              ...
              {page.map((row, i) => {
                ...
                return (
                  <TableRow {...row.getRowProps()}>
                    {row.cells.map(cell => {
                      return <TableCell align='center' padding='none' {...cell.getCellProps()}>{cell.render("Cell")}</TableCell>;
                    })}
                  </TableRow>
                );
              })}
