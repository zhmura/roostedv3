...
import { listUsers, usersByEmail, getUser } from "../../../graphql/queries"
  ...
  const onSubmit = async (formDataCollected) => {
    ...
    try {
      ...
      if(agentSelection === 'roostedSelects') {
        console.log('roosted')
        const updatedReferral = await API.graphql(graphqlOperation(updateReferral, {input: {id: referral.id, referralClientStatus: 'new', referralStatus: 'waitingForAgentAssignment', referralPartnerAgentID: '111-1111-111-111'}}))
  ...
  useEffect(() => {
    ...
    const getPriorAgents = async () => {
      ...
      try {
        ...
        let { data } = await API.graphql(graphqlOperation(listUsers, {
          filter: {or: [{ setupType: { eq: 'partner'}}, {setupType: {eq: 'addedAgent'}}]}, 
          limit: 900000
        }))
   
        if(data.listUsers.items.length > 0) {
          const partnerAgents = data.listUsers.items
        
          setPartnerAgents(partnerAgents)
        }
              ...
              >
                ...
                >
                  <RadioGroup aria-label="position" name="agent" value={agentSelection} onChange={handleChange} row style={{justifyContent: 'center'}}>
                    ...
                    <FormControlLabel
                      ...
                      labelPlacement="top"
                      disabled={partnerAgents.length === 0}
                    />
                  </RadioGroup>
                </Grid>
                <Grid
                ...
                >
                  <Controller
                    ...
                    id="priorAgent"
                    name="priorAgent"
                    helperText={partnerAgents.length === 0 ? 'You have no prior referrals with agents to select from.' : ''}
                    control={control}
                    defaultValue={partnerAgents.length === 0 ? 'No Prior Agents' : partnerAgents[0].referralPartnerAgentID}
                    disabled={partnerAgents.length === 0 || agentSelection === 'roostedSelects' || agentSelection === 'agentSelects'}
                    variant="outlined"
                    margin='dense'
                    as={<TextField 
                          select     
                          ...
                          {partnerAgents.map(agent => (
                            <option
                              key={agent === null || agent === undefined ? 'Deleted' : agent.id}
                              value={agent === null || referral === undefined ? 'Deleted' : agent.id}
                            >
                              {agent === null || agent === undefined ? 'Deleted' : agent.userFirstName + ' ' + agent.userLastName}
                            </option>
                          ))}
                        </TextField>}
                    />
