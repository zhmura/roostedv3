...
import { listPartnerLicenses, getUser } from "../../graphql/queries"
  ...
  const onSubmit = async (dataCollected) => {
    ...
    if(dataCollected.roostedNetworkAgent === 'Select Agent' || dataCollected.roostedNetworkAgent === 'No Agents Found') {
      ...
      try {
          ...
          const roostedAgentEstimatedPayout = estimatePayout('roostedAgent', 0.03, parseFloat(getDefaultReferralFee('roosted')/100), agentShare, referral.buyerPriceRange)
          const partnerAgentEstimatedPayout = estimatePayout('partnerAgent', 0.03, parseFloat(getDefaultReferralFee('partner')/100), agentShare, referral.buyerPriceRange)
          const roostedEstimatedPayout = estimatePayout('roosted', 0.03, parseFloat(getDefaultReferralFee('roosted')/100), agentShare, referral.buyerPriceRange)
          ...
          let fromEmailReferral = getRoostedEmail(referral.buyerState, 'referral')
          ...
          let toEmailRoostedAgent = roostedAgent.email
          let toFullNameRoostedAgent = `${roostedAgent.userFirstName} ${roostedAgent.userLastName}`
          ...
          let fromEmailSupport = getRoostedEmail(referral.buyerState, 'support')

          let toEmailFlock = getRoostedEmail(referral.buyerState, 'flock')
    ...
    const getRoostedNetworkAgents = async () => {
      try {
        ...
        let getPartnerAgents = await API.graphql(graphqlOperation(listPartnerLicenses, {
          limit: 900000,
          filter: {licenseState: { eq: referral.buyerState}},
        }))
        ...
        if(getPartnerAgents.data.listPartnerLicenses.items.length === 0) {
          ...
          for(let i = 0; i < getPartnerAgents.data.listPartnerLicenses.items.length; i++) {
            ...
            for(let n = 0; n < zipCodesByRadius.length; n++) {
              if(zipCodesByRadius[n] === referral.buyerZip) {
      ...
      <CardContent>
        ...
        <Grid
          ...
          <Button
            ...
            disabled={partnerAgentArray.length === 0 || (globalUser.userType !== 'broker' && globalUser.userType !== 'admin')}
            type='submit'
          >
